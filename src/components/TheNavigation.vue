<template>
  <nav class="fixed top-6 left-0 w-full z-100">
    <div class="container-xl">

      <div class="flex justify-between ">
        <div :class="positionY > 40 ? 'size-12' : 'size-28'" class="animate group duration-500">
          <svg width="100%" height="100%" viewBox="0 0 38 48" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_159_290)">
              <path
                d="M25.3063 27.5514C28.0264 26.3424 30.1421 24.8313 31.6533 22.5645C33.1645 20.2977 33.9201 17.7287 33.9201 14.5552C33.9201 9.87055 32.4089 6.39483 29.3865 3.97694C26.3641 1.55904 22.1328 0.350098 16.6926 0.350098H0.220703V47.6501H8.38108V29.2137V22.8667V6.9993H16.9948C19.8661 6.9993 21.9817 7.75489 23.4929 9.11496C25.0041 10.475 25.6086 12.4396 25.6086 15.0086C25.6086 17.5776 24.853 19.391 23.1907 20.7511C21.8306 21.96 19.8661 22.7156 17.4482 22.8667V29.5159L26.9686 47.8012H35.7335V47.3479L25.1552 27.5514H25.3063Z"
                fill="white" />
              <path d="M9.74023 46.8943V8.20801H16.5406V41.9074H22.5853L25.3054 46.8943H9.74023Z" fill="white" />
              <path d="M23.6445 41.1519L27.1202 47.6499H36.0362L32.5605 41.1519H23.6445Z" fill="white" />
              <path d="M37.0944 46.8946L34.5254 41.9077H38.0011V46.8946H37.0944Z" fill="#F78E3F" />
            </g>
            <defs>
              <clipPath id="clip0_159_290">
                <rect width="37.7795" height="47.3" fill="white" transform="translate(0.220703 0.350098)" />
              </clipPath>
            </defs>
          </svg>

        </div>
        <div class="gap-x-4 animate pb-3 border-b h-max nav-items hidden md:flex" :class="positionY > 80 ? 'border-b-transparent' : 'border-b-white/80'">
          <div class="nav-item" data-scroll-to="about">
            Over mij
          </div>
          <div class="nav-item" data-scroll-to="projects">
            Cases
          </div>
<!--          <div class="nav-item" data-scroll-to="contact">-->
<!--           Contact-->
<!--          </div>-->
        </div>
      </div>
    </div>
  </nav>
</template>

<script setup>
import { ref, onMounted } from 'vue'

const positionY = ref(window.scrollY)

onMounted(() => {
  window.addEventListener('scroll', () => {
    positionY.value = window.scrollY
  })
})
</script>

<style lang="postcss" scoped>
.nav-item{
  @apply text-white/80 font-medium hover:text-white cursor-pointer animate;
}
</style>
